package com.leray.android.kiosk.utils;
...
public class ShellUtils {
	...
	public static String MAX_VOLUME_PATH;
	

	public static void listDeviceStorageInfo() {
		new AsyncTask<String, Void, CommandResult>() {
			...
			protected void onPostExecute(CommandResult result) {
				...
				// 默认还是假设系统指定的sdcard容量最大
				File maxVolumeDir = Environment.getExternalStorageDirectory();
				MAX_VOLUME_PATH = maxVolumeDir.getAbsolutePath();
				if (result.result == 0) {
					Log.d(TAG, "success: " + result.successMsg);
					String[] lines = result.successMsg.split("\n");
					for (int i = 1; i < lines.length; i++) {
						Log.d(TAG, "line_" + i + ": " + lines[i]);
						String path = lines[i].split(" ")[0];
						File f = new File(path);
						long totalSpace = f.getTotalSpace();
						if (sdCardTotalSpace < totalSpace) {
							sdCardTotalSpace = totalSpace;
							maxVolumeDir = f;
						}
						Log.d(TAG, "dir_" + i + ": " + path);
					}
					MAX_VOLUME_PATH = maxVolumeDir.getAbsolutePath();
					sdCardFreeSpace = maxVolumeDir.getFreeSpace();
					long totalM = (long) (sdCardTotalSpace * 1.0 / (1024 * 1024));
					long freeM = (long) (sdCardFreeSpace * 1.0 / (1024 * 1024));
					String ret = "max volume dir is: " + maxVolumeDir.getAbsolutePath() + ", total: " + totalM + "M, free: " + freeM + "M.";
					Log.d(TAG, ret);
				} else {
					Log.d(TAG, "获取设备存储信息失败: " + result.errorMsg);
				}
			}
		}.execute();
	}
